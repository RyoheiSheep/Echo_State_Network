...
class ReservoirNetWork:
    ...
    def _get_next_reservoir_nodes(self, input, current_state):
        # print(self.weights_input.shape)
        # print(self.weights_reservoir.shape)
        # print(np.array([input]).shape)       
        
        next_state = (1 - self.leak_rate) * current_state
        # print(next_state.shape)
        # print("###################%%%%%%%%%%%!!!!!!!!!!!")
        # print("shape of input" + str(input.shape))
        # print((np.array([input])@self.weights_input).shape)
        # print("###############################!!!!!!!!!!!!!!!!!!!")
        # print((current_state@self.weights_reservoir).shape)
    ...
    def online_training(self, eta = 0.001,lambda0 = 0.1 ):
            output_weight = np.zeros([self.num_reservoir_nodes, self.num_output_nodes])
            ...
            def loss_function(output_weight):
                ...
                for i in range(len(states)):    
                    s += np.square(teacher[i] - states[i] @ output_weight)
                loss = 1/2*(np.sum(s)/len(states)+np.sum(np.square(output_weight)))    
                return loss
            ...
            print(loss_function(output_weight))        
